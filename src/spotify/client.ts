import { getValidSpotifyToken } from './auth.js';
...
// --- User-specific endpoints (require Premium auth) ---
...
export async function getUserPlaylists(): Promise<SpotifyPlaylist[]> {
  ...
  try {
    ...
    throw new Error(
      'This feature requires Spotify authentication. Run: spyt auth spotify',
    );
  }
  ...
  while (true) {
    const url = new URL(`${API_BASE}/me/playlists`);
    ...
    if (res.status === 403) {
      throw new Error(
        'Listing your playlists requires a Spotify Premium subscription. Use a playlist URL instead: spyt transfer <spotify-url>',
      );
    }
    if (!res.ok) {
...
export async function getLikedTracks(): Promise<SpotifyTrack[]> {
  ...
  while (true) {
    ...
    if (res.status === 403) {
      throw new Error(
        'Accessing liked tracks requires a Spotify Premium subscription.',
      );
    }
    if (!res.ok) {
      throw new Error(`Spotify API error ${res.status}: ${res.statusText}`);
